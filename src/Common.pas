...
//Windows, Forms, Classes, SysUtils, ScktComp;
	Windows, StdCtrls, MMSystem, Classes, SysUtils, ScktComp, List32,tlhelp32;
...
{変更}
	...
	AddSkill   :array[0..330] of Word; //スキル追加
...
type TMobDB = class //edited by The Harbinger -- darkWeiss version
	ID          :word;
	Name        :string;
	JName       :string;
	LV          :byte;
...
type TSlaveDB = class //added by The Harbinger -- darkWeiss version
  Name        :string;
...
//------------------------------------------------------------------------------
// ID,ITEMLIST
type TKafra = class //added by The Harbinger -- darkWeiss version
    PID           :Cardinal;
  	Kafra         :TItemList;
...
//------------------------------------------------------------------------------
// BIP
type TBan = class //added by The Harbinger -- darkWeiss version
    BIP           :string;
end;
//------------------------------------------------------------------------------
...
type TIDTbl = class    //added by The Harbinger -- darkWeiss version
  ID               :integer;
  BroadCast        :integer;
  ItemSummon       :integer;
...
type TMArrowDB = class  //added by The Harbinger -- darkWeiss version
  ID        :integer;
...
//------------------------------------------------------------------------------
// モンスターデータ
type TMob = class //edited by The Harbinger -- darkWeiss version
	ID          :cardinal;
	Name        :string;
	JID         :word;
	Map         :string;
...
//------------------------------------------------------------------------------
//キャラクターデータ
type TChara = class  //edited by The Harbinger -- darkWeiss version
	//制御変数
	ID	          :cardinal;
...
{パーティー機能追加ココまで}
//------------------------------------------------------------------------------
type TCastle = class  //added by The Harbinger -- darkWeiss version
	Name    :string;
  GID     :word;
  GName   :string;
  GMName  :string;
...
//------------------------------------------------------------------------------
type TEmp = class //added by The Harbinger -- darkWeiss version
	Map    :string;
  EID    :cardinal;
end;
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
// NPCデータ
type TNPC = class  //edited by The Harbinger -- darkWeiss version
	ID          :cardinal;
	Name        :string;
{NPCイベント追加}
//	JID         :word;
...
//マップ設定データ
type MapTbl = class //edited by The Harbinger -- darkWeiss version
	noMemo    :Boolean;
	noSave    :Boolean;
	noTele    :Boolean;
  PvP       :Boolean;
...
//------------------------------------------------------------------------------
// 変数
var   //edited by The Harbinger -- darkWeiss version
	AppPath              :string;
...
{キューペットここまで}
  ...
  Days          :cardinal;
	...
	//job_db1
	WeightTable     :array[0..20] of word;
	HPTable         :array[0..20] of word;
	SPTable         :array[0..20] of word;
	WeaponASPDTable :array[0..20] of array[0..16] of word;
	//job_db2
	JobBonusTable   :array[0..20] of array[1..255] of byte;
	//wp_db
...
//------------------------------------------------------------------------------
// 関数定義
		procedure MapLoad(MapName:string);  //edited by The Harbinger -- darkWeiss version
		procedure MapMove(Socket:TCustomWinSocket; MapName:string; Point:TPoint); //edited by The Harbinger -- darkWeiss version
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
		...
		procedure CalcAbility(tc:TChara; td:TItemDB; o:Integer = 0); //edited by The Harbinger -- darkWeiss version
		procedure CalcEquip(tc:TChara);
		procedure CalcSkill(tc:TChara; Tick:cardinal); //edited by The Harbinger -- darkWeiss version
		procedure CalcStat(tc:TChara; Tick:cardinal = 0); //edited by The Harbinger -- darkWeiss version

		procedure CalcLvUP(tc1:TChara; EXP:cardinal; JEXP:cardinal); //edited by The Harbinger -- darkWeiss version
		...
		procedure SendCData(Socket:TCustomWinSocket; tc:TChara; Use0079:boolean = false);
		...
		procedure SendCSkillAtk1(tm:TMap; tc:TChara; ts:TMob; Tick:cardinal; dmg:Word; k:byte; PType:byte = 0);  //edited by The Harbinger -- darkWeiss version
    procedure SendCSkillAtk2(tm:TMap; tc:TChara; tc1:TChara; Tick:cardinal; dmg:Word; k:byte; PType:byte = 0);  //added by The Harbinger -- darkWeiss version
...
{ギルド機能追加}
		...
		procedure SendGLoginInfo(tg:TGuild; tc:TChara);   //edited by The Harbinger -- darkWeiss version
		function  GetGuildConUsers(tg:TGuild) : word;
    procedure GuildDInvest(tn:TNPC); //added by The Harbinger -- darkWeiss version
    procedure SpawnNPCMob(tn:TNPC;MobName:string;X:integer;Y:integer;SpawnDelay1:cardinal;SpawnDelay2:cardinal); //added by The Harbinger -- darkWeiss version
    procedure CallGuildGuard(tn:TNPC;guard:integer); //added by The Harbinger -- darkWeiss version
    procedure SetGuildKafra(tn:TNPC;mode:integer);  //added by The Harbinger -- darkWeiss version
    procedure EnableGuildKafra(MapName:string;KafraName:string;Mode:integer); //added by The Harbinger -- darkWeiss version
    procedure ClaimGuildCastle(ID:cardinal;MapName:string); //added by The Harbinger -- darkWeiss version
    function  GetGuildKafra(tn:TNPC) : integer;  //added by The Harbinger -- darkWeiss version
    function  GetGuildID(tn:TNPC) : word; //added by The Harbinger -- darkWeiss version
    function  GetGuildEDegree(tn:TNPC) : cardinal; //added by The Harbinger -- darkWeiss version
    function  GetGuildETrigger(tn:TNPC) : word; //added by The Harbinger -- darkWeiss version
    function  GetGuildDDegree(tn:TNPC) : cardinal; //added by The Harbinger -- darkWeiss version
    function  GetGuildDTrigger(tn:TNPC) : word; //added by The Harbinger -- darkWeiss version
    function  CheckGuildID(tn:TNPC; tc:TChara) : word; //added by The Harbinger -- darkWeiss version
    function  CheckGuildMaster(tn:TNPC; tc:TChara) : word; //added by The Harbinger -- darkWeiss version
    function  GetGuildName(tn:TNPC) : string;  //added by The Harbinger -- darkWeiss version
    function  GetGuildMName(tn:TNPC) : string; //added by The Harbinger -- darkWeiss version
...
{アイテム製造修正}
					for k := 0 to Item[i].Data.Slot - 1 do begin //カード補正1
						...
						if (Item[i].Card[k] <> 0) and (Item[i].Card[k] > 4000) and (Item[i].Card[k] < 4149) then begin
...
begin
    ...
    end else if (Skill[107].Lv <> 0) then begin //added by The Harbinger -- darkWeiss version
			ATK[0][4] := Skill[107].Data.Data1[Skill[107].Lv];
		end else if (Skill[134].Lv <> 0) and (WeaponType[0] = 16) then begin //カタール
			ATK[0][4] := Skill[134].Data.Data1[Skill[134].Lv];
    end else if (Skill[226].Lv <> 0) and (WeaponType[0] = 6) then begin //added by The Harbinger -- darkWeiss version
      ATK[0][4] := Skill[226].Data.Data1[Skill[226].Lv];
    end else if (Skill[259].Lv <> 0) and (WeaponType[0] = 12) then begin //added by The Harbinger -- darkWeiss version
      ATK[0][4] := Skill[259].Data.Data1[Skill[259].Lv]
    end else if (Skill[274].Lv <> 0) and (WeaponType[0] = 15) then begin //added by The Harbinger -- darkWeiss version
			ATK[0][4] := Skill[274].Data.Data1[Skill[274].Lv];
    end else if (Skill[315].Lv <> 0) and (WeaponType[0] = 13) then begin //added by The Harbinger -- darkWeiss version
      ATK[0][4] := Skill[315].Data.Data1[Skill[315].Lv];
    end else if (Skill[323].Lv <> 0) and (WeaponType[0] = 14) then begin //added by The Harbinger -- darkWeiss version
    ...
    if Skill[107].Lv <> 0 then begin  //added by The Harbinger -- darkWeiss version
      Hit := Hit + Skill[107].Data.Data1[Skill[107].Lv]
    end;
    if Skill[265].Lv <> 0 then begin  //added by The Harbinger -- darkWeiss version
      FLEE1 := FLEE1 + Skill[265].Data.Data1[Skill[265].Lv]
    end;
{追加:119}
		//グロリア(LUK+)
...
{変更}
    ...
    MAXHP := 65535;
		...
		MAXHP := MAXHP + (35 + BaseLV * 5 + ((1 + BaseLV) * BaseLV div 2) * HPTable[JID] div 100) * (100 + Param[2]) div 100;
    end;
...
{追加:119ここまで}
		...
		if ADelay < i then ADelay := 200
			...
			if Skill[111].Tick > Tick then ADelay := ADelay * 70 div 100; //アドレナリンラッシュ(AGI+)
{Editted By AppleGirl}  if Skill[258].Tick > Tick then ADelay := ADelay * 70 div 100; //ツーハンドクイックン
			if ADelay < 200 then ADelay := 200;
		end;
		ASpeed := ADelay div 2;
    if (Option = 32) then begin
...
{:code}
		//移動速度
		i := DefaultSpeed;
		if (Option and 32) <> 0 then begin //ぺこ
			i := i - 30;
		end;
    ...
    //end;
		...
		if Skill[8].Tick > Tick then begin //インデュア
			dMotion := 0;
...
{追加ココまで}
		if Socket <> nil then begin
			if tc.ver2 = 9 then Socket.SendBuf(buf, 53)	//Kr?
...
{追加ココまで}
		if Socket <> nil then begin
			if tc.ver2 = 9 then Socket.SendBuf(buf, 54)	//Kr?
...
{修正}
				...
				if (tc = nil) or (tc <> tc1) then begin
					if (abs(Point.X - tc1.Point.X) < 16) and
					(abs(Point.Y - tc1.Point.Y) < 16) then begin
...
begin
  ...
  Exit;
	...
	with tp do begin
		...
		j := 0;
		for i := 1 to 100 do begin
			if (tk.Kafra.Item[i].ID <> 0) and not tk.Kafra.Item[i].Data.IEquip then begin
				WFIFOW( 4 +j*10, i);
				WFIFOW( 6 +j*10, tk.Kafra.Item[i].Data.ID);
				WFIFOB( 8 +j*10, tk.Kafra.Item[i].Data.IType);
				WFIFOB( 9 +j*10, tk.Kafra.Item[i].Identify);
				WFIFOW(10 +j*10, tk.Kafra.Item[i].Amount);
				if tk.Kafra.Item[i].Data.IType = 10 then
		...
		j := 0;
		for i := 1 to 100 do begin
			if (tk.Kafra.Item[i].ID <> 0) and tk.Kafra.Item[i].Data.IEquip then begin
				WFIFOW( 4 +j*20, i);
				WFIFOW( 6 +j*20, tk.Kafra.Item[i].Data.ID);
				WFIFOB( 8 +j*20, tk.Kafra.Item[i].Data.IType);
				WFIFOB( 9 +j*20, tk.Kafra.Item[i].Identify);
				with tk.Kafra.Item[i].Data do begin
					if (tc.JID = 12) and (IType = 4) and (Loc = 2) and
						 ((View = 1) or (View = 2) or (View = 6)) then
						WFIFOW(10 +j*20, 34)
					else
				...
				WFIFOW(12 +j*20, tk.Kafra.Item[i].Equip);
				WFIFOB(14 +j*20, tk.Kafra.Item[i].Attr);
				WFIFOB(15 +j*20, tk.Kafra.Item[i].Refine);
				WFIFOW(16 +j*20, tk.Kafra.Item[i].Card[0]);
				WFIFOW(18 +j*20, tk.Kafra.Item[i].Card[1]);
				WFIFOW(20 +j*20, tk.Kafra.Item[i].Card[2]);
				WFIFOW(22 +j*20, tk.Kafra.Item[i].Card[3]);
				Inc(j);
				Inc(cnt);
			end;
		end;
...
begin
    ...
    WFIFOW( 8+37*j+4, tc.Skill[i].Data.SP[1]);
      ...
      for k := 0 to 4 do begin
				if (tc.Skill[i].Data.ReqSkill1[k] <> 0) then begin
					c := 1;
				end;
        if (tc.Skill[i].Data.ReqSkill2[k] <> 0) then begin
					d := 1;
				end;
			end;
...
//------------------------------------------------------------------------------
procedure SendCSkillAtk1(tm:TMap; tc:TChara; ts:TMob; Tick:cardinal; dmg:Word; k:byte; PType:byte = 0);
begin
	WFIFOW( 0, $0114);
	WFIFOW( 2, tc.MSkill);
	WFIFOL( 4, tc.ID);
...
//------------------------------------------------------------------------------
procedure SendCSkillAtk2(tm:TMap; tc:TChara; tc1:TChara; Tick:cardinal; dmg:Word; k:byte; PType:byte = 0);
begin
	WFIFOW( 0, $0114);
	WFIFOW( 2, tc.MSkill);
	WFIFOL( 4, tc.ID);
...
begin
  ...
  KafraDB.AddObject(tk.PID,tk);
	...
	if IEquip then begin
		for i := 1 to 100 do begin
			//空きindexを探す
			if tk.Kafra.Item[i].ID = 0 then begin
				tk.Kafra.Item[i].Amount := 0;
				Result := i;
				break;
			end;
		end;
		...
		for i := 1 to 100 do begin
			//同じアイテムを持ってるか探す
			if tk.Kafra.Item[i].ID = ItemID then begin
				Result := i;
				exit;
			end;
		end;
...
begin
	...
	if (tg.EXP >= tg.NextEXP) and (tg.NextEXP <> 0) then begin
		//レベルアップ
...
begin
          ...
          ts.Point := ts.Point1;
					...
					ts.Dir := Random(8);
					ts.HP := ts.Data.HP + (tgc.DTrigger * 2000);
					ts.Speed := ts.Data.Speed;
...
begin
  ...
  MapName := LowerCase(MapName);
	...
	if str <> 'DWM' then begin
		DebugOut.Lines.Add('Mapfile error 500 : ' + MapName + '.dwm');
		tm.Free;
		exit;
	end;
    ...
    ScriptPath := ExtractRelativePath(AppPath, ScriptList.Strings[i1]);
        ...
        sl1.Strings[0] := LowerCase(sl1.Strings[0]);
				if ChangeFileExt(sl1.Strings[0], '') = MapName then begin
      if (ExtractFileExt(sl1.Strings[0]) <> '.dwm') then begin
        DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: Syntax Error: Invalid Map', [ScriptPath, lines]));
			end;
	// ワープポイント ------------------------------------------------------------
...
{NPCイベント追加ココまで}
						...
						while not eof(txt) do begin
							...
							2:	sl1.DelimitedText := sl.Strings[1];
								...
								begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: Syntax Error', [ScriptPath, lines]));
									exit;
								end;
							end;
							...
							if str = 'mes' then begin //------- 1 mes
								if sl1.Count <> 1 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [mes] function error', [ScriptPath, lines]));
									exit;
								end;
								...
								if sl1.Count <> 0 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [next] function error', [ScriptPath, lines]));
									exit;
								end;
								SetLength(tn.Script, k + 1);
								tn.Script[k].ID := 2;
								...
								if sl1.Count <> 0 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [close] function error', [ScriptPath, lines]));
									exit;
								end;
								SetLength(tn.Script, k + 1);
								tn.Script[k].ID := 3;
								...
								if sl1.Count <> sl1.Count div 2 * 2 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [menu] function error', [ScriptPath, lines]));
									exit;
								end;
								if sl1.Count > 40 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [menu] too many selections', [ScriptPath, lines]));
									exit;
								end;
								SetLength(tn.Script, k + 1);
								tn.Script[k].ID := 4;
								...
								if sl1.Count <> 1 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [goto] function error', [ScriptPath, lines]));
									exit;
								end;
								SetLength(tn.Script, k + 1);
								tn.Script[k].ID := 5;
								...
								if sl1.Count <> 2 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [cutin] function error', [ScriptPath, lines]));
									exit;
								end;
								val(sl1.Strings[1], i, j);
								if (j <> 0) or (i < 0) or (i > 255) then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [cutin] range error (2)', [ScriptPath, lines]));
									exit;
								end;
								SetLength(tn.Script, k + 1);
								tn.Script[k].ID := 6;
								...
								if sl1.Count <> 0 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [store] function error', [ScriptPath, lines]));
									exit;
								end;
								SetLength(tn.Script, k + 1);
								tn.Script[k].ID := 7;
								...
								if sl1.Count <> 3 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [warp] function error', [ScriptPath, lines]));
									exit;
								end;
								val(sl1.Strings[1], i, j);
								if (j <> 0) or (i < 0) or (i > 511) then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [warp] range error (2)', [ScriptPath, lines]));
									exit;
								end;
								val(sl1.Strings[2], i, j);
								if (j <> 0) or (i < 0) or (i > 511) then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [warp] range error (3)', [ScriptPath, lines]));
									exit;
								end;
								SetLength(tn.Script, k + 1);
								tn.Script[k].ID := 8;
								...
								if sl1.Count <> 3 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [save] function error', [ScriptPath, lines]));
									exit;
								end;
								val(sl1.Strings[1], i, j);
								if (j <> 0) or (i < 0) or (i > 511) then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [save] range error (2)', [ScriptPath, lines]));
									exit;
								end;
								val(sl1.Strings[2], i, j);
								if (j <> 0) or (i < 0) or (i > 511) then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [save] range error (3)', [ScriptPath, lines]));
									exit;
								end;
								SetLength(tn.Script, k + 1);
								tn.Script[k].ID := 9;
								...
								if sl1.Count <> 2 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [heal] function error', [ScriptPath, lines]));
									exit;
								end;
								val(sl1.Strings[0], i, j);
								if (j <> 0) or (i < 0) or (i > 30000) then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [heal] range error (1)', [ScriptPath, lines]));
									exit;
								end;
								val(sl1.Strings[1], i, j);
								if (j <> 0) or (i < 0) or (i > 30000) then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [heal] range error (2)', [ScriptPath, lines]));
									exit;
								end;
								SetLength(tn.Script, k + 1);
								tn.Script[k].ID := 10;
								...
								if sl1.Count <> 4 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [set] function error', [ScriptPath, lines]));
									exit;
								end;
								val(sl1.Strings[1], i, j);
								if (j <> 0) or (i < 0) or (i > 4) then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [set] range error (2)', [ScriptPath, lines]));
									exit;
								end;
								val(sl1.Strings[2], i, j);
								if j = 0 then begin
									if (i < -999999999) or (i > 999999999) then begin
										DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [set] range error (3)', [ScriptPath, lines]));
										exit;
									end else if (StrToInt(sl1.Strings[1]) = 3) and (i = 0) then begin
										DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [set] div 0 error (3)', [ScriptPath, lines]));
										exit;
									end;
								end;
								val(sl1.Strings[3], i, j);
								if (j <> 0) or (i < -999999999) or (i > 999999999) then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [set] range error (4)', [ScriptPath, lines]));
									exit;
								end;
								SetLength(tn.Script, k + 1);
								tn.Script[k].ID := 11;
								...
								if sl1.Count <> 2 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [additem] function error', [ScriptPath, lines]));
									exit;
								end;
								val(sl1.Strings[0], i, j);
								if j = 0 then begin
									if (i < 0) or (i > 19999) then begin
										DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [additem] range error (1)', [ScriptPath, lines]));
										exit;
									end;
								end;
								val(sl1.Strings[1], i, j);
								if j = 0 then begin
									if (i < 0) or (i > 30000) then begin
										DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [additem] range error (2)', [ScriptPath, lines]));
										exit;
									end;
								end;
								SetLength(tn.Script, k + 1);
								...
								if sl1.Count <> 2 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [delitem] function error', [ScriptPath, lines]));
									exit;
								end;
								val(sl1.Strings[0], i, j);
								if j = 0 then begin
									if (i < 0) or (i > 19999) then begin
										DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [delitem] range error (1)', [ScriptPath, lines]));
										exit;
									end;
								end;
								val(sl1.Strings[1], i, j);
								if j = 0 then begin
									if (i < 0) or (i > 30000) then begin
										DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [delitem] range error (2)', [ScriptPath, lines]));
										exit;
									end;
								end;
								SetLength(tn.Script, k + 1);
								...
								if sl1.Count <> 4 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [checkitem] function error', [ScriptPath, lines]));
									exit;
								end;
								val(sl1.Strings[0], i, j);
								if j = 0 then begin
									if (i < 0) or (i > 19999) then begin
										DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [checkitem] range error (1)', [ScriptPath, lines]));
										exit;
									end;
								end;
								val(sl1.Strings[1], i, j);
								if j = 0 then begin
									if (i < 0) or (i > 30000) then begin
										DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [checkitem] range error (2)', [ScriptPath, lines]));
										exit;
									end;
								end;
								SetLength(tn.Script, k + 1);
								...
								if sl1.Count <> 5 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [check] function error', [ScriptPath, lines]));
									exit;
								end;
								val(sl1.Strings[2], i, j);
								if j = 0 then begin
									if (i < 0) or (i > 999999999) then begin
										DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [check] range error (1)', [ScriptPath, lines]));
										exit;
									end;
								end;
								SetLength(tn.Script, k + 1);
								...
								if j = -1 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [check] syntax error (2)', [ScriptPath, lines]));
									exit;
								end;
								...
								if sl1.Count <> 4 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [checkadditem] function error', [ScriptPath, lines]));
									exit;
								end;
								val(sl1.Strings[0], i, j);
								if j = 0 then begin
									if (i < 0) or (i > 19999) then begin
										DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [checkadditem] range error (1)', [ScriptPath, lines]));
										exit;
									end;
								end;
								val(sl1.Strings[1], i, j);
								if j = 0 then begin
									if (i < 0) or (i > 30000) then begin
										DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [checkadditem] range error (2)', [ScriptPath, lines]));
										exit;
									end;
								end;
								SetLength(tn.Script, k + 1);
								...
								if sl1.Count <> 1 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [jobchange] function error', [ScriptPath, lines]));
									exit;
								end;
								val(sl1.Strings[0], i, j);
								if (j <> 0) or (i < 0) or (i > 44) then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [jobchange] range error (1)', [ScriptPath, lines]));
									exit;
								end;
								SetLength(tn.Script, k + 1);
								tn.Script[k].ID := 17;
								...
								if sl1.Count <> 5 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [viewpoint] function error', [ScriptPath, lines]));
									exit;
								end;
								val(sl1.Strings[0], i, j);
								if (j <> 0) or (i < 1) or (i > 2) then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [viewpoint] range error (1)', [ScriptPath, lines]));
									exit;
								end;
								val(sl1.Strings[1], i, j);
								if (j <> 0) or (i < 0) or (i > 511) then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [viewpoint] range error (2)', [ScriptPath, lines]));
									exit;
								end;
								val(sl1.Strings[2], i, j);
								if (j <> 0) or (i < 0) or (i > 511) then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [viewpoint] range error (3)', [ScriptPath, lines]));
									exit;
								end;
								val(sl1.Strings[3], i, j);
								if (j <> 0) or (i < 0) or (i > 255) then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [viewpoint] range error (4)', [ScriptPath, lines]));
									exit;
								end;
								...
								if sl1.Count <> 1 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [input] function error', [ScriptPath, lines]));
									exit;
								end;
								SetLength(tn.Script, k + 1);
								tn.Script[k].ID := 19;
								...
								if sl1.Count <> 2 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [random] function error', [ScriptPath, lines]));
									exit;
								end;
								val(sl1.Strings[0], i, j);
								if j = 0 then begin
									if (i < 0) or (i > 999999999) then begin
										DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [random] range error (1)', [ScriptPath, lines]));
										exit;
									end;
								end;
								val(sl1.Strings[1], i, j);
								if (j <> 0) or (i < 0) or (i > 999999999) then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [random] range error (2)', [ScriptPath, lines]));
									exit;
								end;
								SetLength(tn.Script, k + 1);
								tn.Script[k].ID := 20;
								...
								if sl1.Count <> 2 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [option] function error', [ScriptPath, lines]));
									exit;
								end;
								val(sl1.Strings[0], i, j);
								if (j <> 0) or (i < 0) or (i > 2) then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [option] range error (1)', [ScriptPath, lines]));
									exit;
								end;
								val(sl1.Strings[1], i, j);
								if (j <> 0) or (i < 0) or (i > 1) then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [option] range error (2)', [ScriptPath, lines]));
									exit;
								end;
								SetLength(tn.Script, k + 1);
								tn.Script[k].ID := 21;
								...
								if sl1.Count <> 1 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [speed] function error', [ScriptPath, lines]));
									exit;
								end;
								val(sl1.Strings[0], i, j);
								if j = 0 then begin
									if (i < 25) or (i > 1000) then begin
										DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [speed] range error (1)', [ScriptPath, lines]));
										exit;
									end;
								end;
								SetLength(tn.Script, k + 1);
								...
								if sl1.Count <> 0 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [die] function error', [ScriptPath, lines]));
									exit;
								end;
								SetLength(tn.Script, k + 1);
								tn.Script[k].ID := 23;
								...
								if sl1.Count <> 1 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [ccolor] function error', [ScriptPath, lines]));
									exit;
								end;
								{val(sl1.Strings[0], i, j);
								if (j <> 0) or (i > 5) then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [ccolor] range error (1)', [ScriptPath, lines]));
									exit;
								end;}
								SetLength(tn.Script, k + 1);
								tn.Script[k].ID := 24;
								...
								if sl1.Count <> 3 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [refine] function error', [ScriptPath, lines]));
									exit;
								end;
								//val(sl1.Strings[0], i, j);
								//val(sl1.Strings[1], i, j);
								...
								if (j <> 0) or (i > 10) then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [refine] range error (1)', [ScriptPath, lines]));
									exit;
								end;
								SetLength(tn.Script, k + 1);
								tn.Script[k].ID := 25;
								...
								if sl1.Count <> 2 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [getitemamount] function error', [ScriptPath, lines]));
									exit;
								end;
								val(sl1.Strings[0], i, j);
								if j = 0 then begin
									if (i < 0) or (i > 19999) then begin
										DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [getitemamount] range error (1)', [ScriptPath, lines]));
										exit;
									end;
								end;
								SetLength(tn.Script, k + 1);
...
{追加:スクリプト144}
								...
								if sl1.Count <> 2 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [getskilllevel] function error', [ScriptPath, lines]));
									exit;
								end;
								...
								if sl1.Count <> 2 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [setskilllevel] function error', [ScriptPath, lines]));
									exit;
								end;
								val(sl1.Strings[1], i, j);
								if j = 0 then begin
									if (i < 0) or (i > 10) then begin
										DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [setskilllevel] range error(2)', [ScriptPath, lines]));
										exit;
									end;
								end;
								SetLength(tn.Script, k + 1);
...
{精錬NPC機能追加}
								...
								if sl1.Count <> 3 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [refinery] function error', [ScriptPath, lines]));
									exit;
								end;
								val(sl1.Strings[0], i, j);
								if j = 0 then begin
									if (i < 0) or (i > 10) then begin
										DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [refinery] range error (1)', [ScriptPath, lines]));
										exit;
									end;
								end;
								val(sl1.Strings[1], i, j);
								if j = 0 then begin
									if (i < 0) or (i > 2) then begin
										DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [refinery] range error (2)', [ScriptPath, lines]));
										exit;
									end;
								end;
								val(sl1.Strings[2], i, j);
								if j = 0 then begin
									if (i < 0) or (i > 10) then begin
										DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [refinery] range error (3)', [ScriptPath, lines]));
										exit;
									end;
								end;
								SetLength(tn.Script, k + 1);
								...
								if sl1.Count <> 3 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [equipmenu] function error', [ScriptPath, lines]));
									exit;
								end;
								SetLength(tn.Script, k + 1);
								tn.Script[k].ID := 30;
								...
								if sl1.Count <> 1 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [lockitem] function error', [ScriptPath, lines]));
									exit;
								end;
								val(sl1.Strings[0], i, j);
								if j = 0 then begin
									if (i < 0) or (i > 1) then begin
										DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [lockitem] range error', [ScriptPath, lines]));
										exit;
									end;
								end;
								SetLength(tn.Script, k + 1);
...
{髪色変更追加}
								...
								if sl1.Count <> 1 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [hcolor] function error', [ScriptPath, lines]));
									exit;
								end;
								val(sl1.Strings[0], i, j);
								if j = 0 then begin
									if (i < 0) or (i > 8) then begin
										DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [hcolor] range error', [ScriptPath, lines]));
										exit;
									end;
								end;
								SetLength(tn.Script, k + 1);
...
{NPCイベント追加}
								...
								if (sl1.Count <> 7) then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [callmob] function error', [ScriptPath, lines]));
									exit;
								end;
								i := 0;
								...
								if i <> 0 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [callmob] range error', [ScriptPath, lines]));
									exit;
								end;
								SetLength(tn.Script, k + 1);
								tn.Script[k].ID := 33;
								...
								if (sl1.Count <> 2) then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [broadcast] function error', [ScriptPath, lines]));
									exit;
								end;
								SetLength(tn.Script, k + 1);
								tn.Script[k].ID := 34;
								...
								if (sl1.Count <> 1) then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [npctimer] function error', [ScriptPath, lines]));
									exit;
								end;
								val(sl1.Strings[0], i, j);
								if j = 0 then begin
									if (i < 0) or (i > 8) then begin
										DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [npctimer] range error', [ScriptPath, lines]));
										exit;
									end;
								end;
								SetLength(tn.Script, k + 1);
								...
								if sl1.Count <> 2 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [addnpctimer] function error', [ScriptPath, lines]));
									exit;
								end;
								val(sl1.Strings[1], i, j);
								if (j <> 0) or (i < -999999999) or (i > 999999999) then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [addnpctimer] range error', [ScriptPath, lines]));
									exit;
								end;
								SetLength(tn.Script, k + 1);
								tn.Script[k].ID := 36;
								...
								if sl1.Count <> 0 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [return] function error', [ScriptPath, lines]));
									exit;
								end;
								SetLength(tn.Script, k + 1);
								tn.Script[k].ID := 37;
								...
								if (sl1.Count = 3) then sl1.Add('0');
								if (sl1.Count <> 4) then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [warpallpc] function error', [ScriptPath, lines]));
									exit;
								end;
								val(sl1.Strings[1], i, j);
								if (j <> 0) or (i < 0) or (i > 511) then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [warpallpc] range error (2)', [ScriptPath, lines]));
									exit;
								end;
								val(sl1.Strings[2], i, j);
								if (j <> 0) or (i < 0) or (i > 511) then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [warpallpc] range error (3)', [ScriptPath, lines]));
									exit;
								end;
								val(sl1.Strings[3], i, j);
								if (j <> 0) or (i < 0) or (i > 2) then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [warpallpc] range error (4)', [ScriptPath, lines]));
									exit;
								end;
								SetLength(tn.Script, k + 1);
								tn.Script[k].ID := 38;
								...
								if (sl1.Count <> 2) then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [waitingroom] function error', [ScriptPath, lines]));
									exit;
								end;
								val(sl1.Strings[1], i, j);
								if (j <> 0) or (i < 2) or (i > 20) then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [waitingroom] range error (2)', [ScriptPath, lines]));
									exit;
								end;
								SetLength(tn.Script, k + 1);
								tn.Script[k].ID := 39;
								...
								if (sl1.Count <> 3) then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [enablenpc] function error', [ScriptPath, lines]));
									exit;
								end;
								val(sl1.Strings[2], i, j);
								if (j <> 0) or (i < 0) or (i > 1) then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [enablenpc] range error (2)', [ScriptPath, lines]));
									exit;
								end;
								SetLength(tn.Script, k + 1);
								tn.Script[k].ID := 40;
								...
								if (sl1.Count <> 1) then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [resetmymob] function error', [ScriptPath, lines]));
									exit;
								end;
								SetLength(tn.Script, k + 1);
								tn.Script[k].ID := 41;
								...
								if (sl1.Count <> 2) then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [getmapusers] function error', [ScriptPath, lines]));
									exit;
								end;
								SetLength(tn.Script, k + 1);
								tn.Script[k].ID := 42;
								...
								sl1.DelimitedText := sl.Strings[0];
								if (sl1.Count <> 3) then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [setstr] function error', [ScriptPath, lines]));
									exit;
								end;
								if (Copy(sl1.Strings[0], 1, 1) <> '$') and (Copy(sl1.Strings[0], 1, 2) <> '\$') then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [setstr] range error (1)', [ScriptPath, lines]));
									exit;
								end;
								val(sl1.Strings[1], i, j);
								if (j <> 0) or (i < 0) or (i > 1) then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [setstr] range error (2)', [ScriptPath, lines]));
									exit;
								end;
								SetLength(tn.Script, k + 1);
								tn.Script[k].ID := 43;
								...
								sl1.DelimitedText := sl.Strings[0];
								if sl1.Count <> 5 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [checkstr] function error', [ScriptPath, lines]));
									exit;
								end;
								SetLength(tn.Script, k + 1);
								tn.Script[k].ID := 44;
								...
								if j = -1 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [checkstr] syntax error (2)', [ScriptPath, lines]));
									exit;
								end;
								tn.Script[k].Data3[2] := j;
								tn.Script[k].Data2[0] := LowerCase(sl1.Strings[3]);
...
{NPCイベント追加ココまで}
								...
								if sl1.Count <> 0 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [resetstat] function error', [ScriptPath, lines]));
									exit;
								end;
								SetLength(tn.Script, k + 1);
								tn.Script[k].ID := 45;
								...
								if sl1.Count <> 0 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [resetskill] function error', [ScriptPath, lines]));
									exit;
								end;
								SetLength(tn.Script, k + 1);
								tn.Script[k].ID := 46;
								...
								if sl1.Count <> 1 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [hstyle] function error', [ScriptPath, lines]));
									exit;
								end;
								val(sl1.Strings[0], i, j);
								if j = 0 then begin
									if (i < 0) or (i > 19) then begin
										DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [hstyle] range error', [ScriptPath, lines]));
										exit;
									end;
								end;
								SetLength(tn.Script, k + 1);
								...
								if sl1.Count <> 1 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [guildreg] function error', [ScriptPath, lines]));
									exit;
								end;
                //j := CastleList.IndexOf(sl1.Strings[0]);
								//if j = - 1 then begin
                  //DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [guildreg] map error', [ScriptPath, lines]));
                  //exit;
								//end;
								SetLength(tn.Script, k + 1);
								tn.Script[k].ID := 48;
								...
								if sl1.Count <> 2 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [getgskilllevel] function error', [ScriptPath, lines]));
									exit;
								end;
								...
								if sl1.Count <> 2 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [getguardstatus] function error', [ScriptPath, lines]));
									exit;
								end;
								...
								if sl1.Count <> 1 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [setguildkafra] function error', [ScriptPath, lines]));
									exit;
								end;
								val(sl1.Strings[0], i, j);
								if j = 0 then begin
									if (i < 0) or (i > 1) then begin
										DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [setguildkafra] range error', [ScriptPath, lines]));
										exit;
									end;
								end;
								SetLength(tn.Script, k + 1);
								...
								if sl1.Count <> 2 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [setguardstatus] function error', [ScriptPath, lines]));
									exit;
								end;
                val(sl1.Strings[0], i, j);
								if j = 0 then begin
									if (i < 1) or (i > 8) then begin
										DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [setguardstatus] range error', [ScriptPath, lines]));
										exit;
									end;
								end;
                val(sl1.Strings[1], i, j);
								...
								if sl1.Count <> 1 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [callguard] function error', [ScriptPath, lines]));
									exit;
								end;
								val(sl1.Strings[0], i, j);
								if j = 0 then begin
									if (i < 1) or (i > 8) then begin
										DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [callguard] range error', [ScriptPath, lines]));
										exit;
									end;
								end;
								SetLength(tn.Script, k + 1);
								...
								if sl1.Count <> 5 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [callmymob] function error', [ScriptPath, lines]));
									exit;
								end;
                ...
                tn.Script[k].Data1[4] := LowerCase(sl1.Strings[4]);
								...
								if sl1.Count <> 0 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [resetguild] function error', [ScriptPath, lines]));
									exit;
								end;
								SetLength(tn.Script, k + 1);
								tn.Script[k].ID := 55;
								...
								if sl1.Count <> 0 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [guilddinvest] function error', [ScriptPath, lines]));
									exit;
								end;
								SetLength(tn.Script, k + 1);
								tn.Script[k].ID := 56;
								...
								if sl1.Count <> 1 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [script] function error', [ScriptPath, lines]));
									exit;
								end;
								SetLength(tn.Script, k + 1);
								tn.Script[k].ID := 99;
...
{NPCイベント追加}
								...
								if sl1.Count <> 1 then begin
									DebugOut.Lines.Add(Format('***ERROR LOADING SCRIPT:%s Line #%.4d: [scriptlabel] function error', [ScriptPath, lines]));
									exit;
								end;
