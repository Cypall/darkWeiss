...
{キューペットここまで}
	...
	Caption := 'darkWeiss'; //ExtractFileName(ChangeFileExt(ParamStr(0), ''));
        ...
        258:
          begin
            ...
            tc.MMode := 4;
					...
					begin
            if (tm.CList.IndexOf(tc.MTarget) <> -1) and (mi.PvP = false) then begin
								MMode := 4;
								MTarget := 0;
                Exit;
           end;
					...
					begin
						...
						for j1 := (xy.Y - tl.Range2) div 8 to (xy.Y + tl.Range2) div 8 do begin
							for i1 := (xy.X - tl.Range2) div 8 to (xy.X + tl.Range2) div 8 do begin
								for k1 := 0 to tm.Block[i1][j1].CList.Count - 1 do begin
									tc2 := tm.Block[i1][j1].CList.Objects[k1] as TChara;
									if (tc1 = tc2) or (tc = tc2)  then Continue;
									if (abs(tc2.Point.X - xy.X) <= tl.Range2) and (abs(tc2.Point.Y - xy.Y) <= tl.Range2) then
										sl.AddObject(IntToStr(tc2.ID),tc2);
								end;
							end;
...
//反撃モード
	with ts do begin
		tc1 := AData;
		if ( (tc1.Sit = 1) or (tc1.Login <> 2) or (tc1.Option >= 64) or (tc1.Option = 6) ) and (ts.isLooting = false) then begin
...
{U0x003b}
			try
				...
				if NextFlag and (DmgTick <= Tick) then begin
					...
					if (tc.MMode = 0) and (tm.gat[NextPoint.X][NextPoint.Y] <> 0) and (tc.Option <> 6) then begin
						//追加移動
						AMode := 0;
...
{修正}
          ...
          if (tm.CList.IndexOf(tc.ATarget) <> -1) and (mi.Pvp = true) then begin
